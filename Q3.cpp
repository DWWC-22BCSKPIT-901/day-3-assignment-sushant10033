Q.3. You are given an integer array nums. Two players are playing a game with this 
...
end of the array (i.e., nums[0] or nums[nums.length - 1]) which reduces the size of 
the array by 1. The player adds the chosen number to their score. The game ends 
...
{     int dp[n][n];     for (int i = 0; i < 
n; i++) {         dp[i][i] = nums[i];  
    }  
    for (int length = 2; length <= n; length++) {         for (int i = 0; i 
<= n - length; i++) {             int j = i + length - 1;             dp[i][j] = 
...
cin >> n;     int nums[n];     cout << "Enter the elements 
of the array: ";     for (int i = 0; i < n; i++) {         cin >> 
nums[i];  
    }  
    if (canPlayer1Win(nums, n)) {  
        cout << "Player 1 can win the game." << endl;  
} else {         
cout << "Player 1 cannot win the 
game." << endl;  
}     
return 0; } 
